...
function setValues(pokemon) {
  ...
  const elPokemontypes = document.getElementById("pokemon-types");
  ...
  elPokemontypes.innerHTML = pokemon ? pokemon.types : '-'
...
function getPokemon(pokemonId) {
  ...
  service.getItem(pokemonId).then(response => {
    const { name, abilities, height, base_experience, sprites, weight, types} = response
    const pokemon = new Pokemon(name, abilities, height, base_experience, sprites, weight, types)
    setValues(pokemon)
  })
}
